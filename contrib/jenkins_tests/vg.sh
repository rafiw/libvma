...
for test_link in $test_ip_list; do
	for test in $test_list; do
		...
		eval "LD_PRELOAD=$test_lib \
			timeout -s SIGINT 2m valgrind --log-file=${vg_dir}/${test_name}-valgrind-sr.log $vg_args \
			$test_app sr ${test_opt} -i ${test_ip} > /dev/null 2>&1 &"
		sleep 15
		eval "LD_PRELOAD=$test_lib \
			timeout -s SIGINT 2m valgrind --log-file=${vg_dir}/${test_name}-valgrind-cl.log $vg_args \
			$test_app pp ${test_opt} -i ${test_ip} -t 10"

		# in case SIGHUP didn't work
		if pgrep -x $test_app > /dev/null
		then
			sudo pkill -15 $test_app
			sleep 2
			if pgrep -x $test_app > /dev/null
			then
				sudo pkill -9 $test_app
			fi
		fi
		sleep 5
